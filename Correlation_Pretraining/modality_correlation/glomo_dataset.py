...
HUMOR_VISUAL_FEATURES = list(range(55, 91))
HUMOR_ACOUSTIC_FEATURES = list(range(0, 60))
HUMOR_VISUAL_DIM_ALL = 91
HUMOR_ACOUSTIC_DIM_ALL = 81
...
class GLoMoHumorPKLDataset(Dataset):
    def __init__(
        self,
        pkl_path: str,
        split: str,
        ...
        if cache_dir:
            ...
            self.cache_path = os.path.join(
                cache_dir,
                f"{base_name}.{split}.humor.seq{max_seq_length}.pt",
            )
    ...
    def _get_inversion(self, tokens, spiece_marker="_"):
        inversion_index = -1
        inversions = []
        for token in tokens:
    ...
    def _build_sequence(self, sample):
        (p_words, p_visual, p_acoustic, _p_hcf), (c_words, c_visual, c_acoustic, _c_hcf), hid, label = sample
        ...
        inversions_a = self._get_inversion(tokens_a)
        inversions_b = self._get_inversion(tokens_b)
        pop_count = self._truncate_seq_pair(tokens_a, tokens_b, self.max_seq_length - 3)
        inversions_a = inversions_a[pop_count:]
        inversions_b = inversions_b[: len(tokens_b)]

        visual_a = np.array([c_visual[inv_id, :] for inv_id in inversions_a]) if len(inversions_a) else np.zeros((0, HUMOR_VISUAL_DIM_ALL))
        acoustic_a = np.array([c_acoustic[inv_id, :] for inv_id in inversions_a]) if len(inversions_a) else np.zeros((0, HUMOR_ACOUSTIC_DIM_ALL))
        visual_b = np.array([p_visual[inv_id, :] for inv_id in inversions_b]) if len(inversions_b) else np.zeros((0, HUMOR_VISUAL_DIM_ALL))
        acoustic_b = np.array([p_acoustic[inv_id, :] for inv_id in inversions_b]) if len(inversions_b) else np.zeros((0, HUMOR_ACOUSTIC_DIM_ALL))

        tokens = [self.tokenizer.cls_token] + tokens_a + [self.tokenizer.sep_token] + tokens_b + [self.tokenizer.sep_token]
        segment_ids = [0] * (len(tokens_a) + 2) + [1] * (len(tokens_b) + 1)

        acoustic_zero = np.zeros((1, HUMOR_ACOUSTIC_DIM_ALL))
        visual_zero = np.zeros((1, HUMOR_VISUAL_DIM_ALL))
        if len(tokens_a) == 0:
            acoustic = np.concatenate((acoustic_zero, acoustic_zero, acoustic_b, acoustic_zero))
            visual = np.concatenate((visual_zero, visual_zero, visual_b, visual_zero))
        else:
            acoustic = np.concatenate((acoustic_zero, acoustic_a, acoustic_zero, acoustic_b, acoustic_zero))
            visual = np.concatenate((visual_zero, visual_a, visual_zero, visual_b, visual_zero))

        acoustic = np.take(acoustic, HUMOR_ACOUSTIC_FEATURES, axis=1)
        visual = np.take(visual, HUMOR_VISUAL_FEATURES, axis=1)
        return tokens, segment_ids, acoustic, visual, hid, label

    def _embed_tokens(self, tokens, segment_ids, device):
        input_ids = self.tokenizer.convert_tokens_to_ids(tokens)
        input_ids = torch.tensor(input_ids, dtype=torch.long, device=device).unsqueeze(0)
        token_type_ids = torch.tensor(segment_ids, dtype=torch.long, device=device).unsqueeze(0)
        attention_mask = torch.ones_like(input_ids)
    ...
    def _build_text_cache(self):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self._bert_model = BertModel.from_pretrained(self.bert_path)
        self._bert_model.to(device)
        self._bert_model.eval()
        cache = []
        for sample in self.samples:
            tokens, segment_ids, _acoustic, _visual, _hid, _label = self._build_sequence(sample)
            cache.append(self._embed_tokens(tokens, segment_ids, device))
        del self._bert_model
        return cache

    def _get_text(self, index, tokens, segment_ids):
        if self.text_cache is None:
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            model = BertModel.from_pretrained(self.bert_path)
            model.to(device)
    ...
    def generate_negative_sample(self, index, text_pos, audio_pos, vision_pos):
        ...
        chosen_strategy = random.choice(["A", "B", "C"])
        if chosen_strategy == "A":
            chosen_modality = random.choice(["T", "A", "V"])
            rand_idx = random.randint(0, len(self.samples) - 1)
            while rand_idx == index:
                rand_idx = random.randint(0, len(self.samples) - 1)
            tokens_r, segment_ids_r, acoustic_r, visual_r, _hid_r, _label_r = self._build_sequence(self.samples[rand_idx])
            other_text = self._get_text(rand_idx, tokens_r, segment_ids_r)
            other_audio = torch.tensor(acoustic_r, dtype=torch.float)
            other_vision = torch.tensor(visual_r, dtype=torch.float)
            if chosen_modality == "T":
                text_neg = other_text.clone()
    ...
    def __getitem__(self, index):
        tokens, segment_ids, acoustic, visual, hid, label = self._build_sequence(self.samples[index])
        text = self._get_text(index, tokens, segment_ids)
        audio = torch.tensor(acoustic, dtype=torch.float)
        vision = torch.tensor(visual, dtype=torch.float)
        label = torch.tensor(label, dtype=torch.float)

        if not self.for_correlation:
            return ((hid, text, audio, vision), label, (hid,))
        ...
        return (
            (hid, text_pos, audio_pos, vision_pos),
            (text_neg, audio_neg, vision_neg),
            label,
            (hid,),
        )
